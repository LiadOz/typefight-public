...
from backend.players import (IPlayer, mediator_factory, PlayerQueueAndMode,
                             PlayerMediator)


class TestDuelMediator(unittest.TestCase):
    def setUp(self):
        self.mediator = mediator_factory(2)
        self.p1 = IPlayer(self.mediator, 0)
        self.p2 = IPlayer(self.mediator, 1)

    def test_register(self):
        self.assertEqual(self.p1, self.mediator.player_1)
        self.assertEqual(self.p2, self.mediator.player_2)
        self.assertEqual(self.mediator.registered, 2)

    def test_opponent(self):
        self.assertEqual(self.mediator.get_opponent(self.p1), self.p2)
        self.assertEqual(self.mediator.get_opponent(self.p2), self.p1)
        with self.assertRaises(Exception):
            p3 = IPlayer()
            self.mediator.get_opponent(p3)

    def test_p1_to_p2(self):
        self.p2.recieve_action = MagicMock()
        self.p1.mediator.send_action(self.p1, 'a')
        self.p2.recieve_action.assert_called_once_with('a')

    def test_p2_to_p1(self):
        self.p1.recieve_action = MagicMock()
        self.p2.mediator.send_action(self.p2, 'a')
